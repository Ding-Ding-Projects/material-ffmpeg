...
This pass intentionally ran no tests, lint, static analysis, accessibility checks, captures, or installer execution. The GUI's Parallel jobs preference is wired through the trusted preload/main-process boundary to the live scheduler with the scheduler's real one-through-four bound, but that interaction remains unrun. Inspector JSON/CSV/XML export is now bound to the exact retained inspection snapshot, uses only a trusted output handle, documents its nested JSON and row-per-node CSV/XML behavior, and enforces a 32 MiB serialization limit; those export interactions also remain unrun. Trim, Filtergraph, audio extraction, two-pass loudness normalization, GIF, thumbnails, HLS, RTMP/SRT, Composer, batch conversion, additional queue controls, other catalog kinds, and component help remain unrun rather than inferred working. The offline documentation and Ollama desktop bridges plus the broader universal feature contract remain incomplete.
