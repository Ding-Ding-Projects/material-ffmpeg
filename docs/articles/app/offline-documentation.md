...
The search field is plain-text-first and bounded to 240 characters. Its adjacent **Regex builder** opens an anchored popover where users can enter a pattern and flags; patterns are compiled before being applied and invalid expressions remain a clear error rather than silently matching nothing. Article navigation works with mouse, <kbd>ArrowUp</kbd>/<kbd>ArrowDown</kbd>, and <kbd>Enter</kbd>. The selected article can be copied to the clipboard or exported as Markdown through a browser download. Both actions are disabled until a real article has loaded.
