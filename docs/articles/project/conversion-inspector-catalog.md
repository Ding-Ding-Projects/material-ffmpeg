...
The renderer provides guided conversion, trimming, filtergraph, audio, GIF, thumbnail, HLS, command-composer, preset, and batch-converter surfaces. Typed builders produce argument arrays for trusted execution. The filtergraph surface keeps video and audio nodes in separate ordered chains, provides per-filter guided controls, and queues only node kinds, names, and option objects accepted by the builder allowlist. The audio extraction surface inspects the selected input, requires one detected audio stream, offers Matroska stream copy or bounded M4A/AAC, MP3, Ogg Opus, FLAC, and WAV/24-bit PCM formats, and selects its output through the native save dialog. Re-encoding formats expose only fixed bitrate choices where the encoder uses them, fixed source/44.1/48/96/192 kHz sample-rate choices, and source/mono/stereo channel choices. GIF output uses one in-memory palette-generation and palette-use graph; per-frame palette statistics also enable a new palette for each frame. Thumbnail output seeks to one selected timestamp and limits the job to one JPEG or PNG still rather than routing a single save destination through a sequence workflow. The batch converter maps inspected media inputs to MP4, MKV, WebM, MP3, FLAC, WAV, PNG, and JPEG adapters.

Batch selection and inspection stay behind a dedicated trusted boundary. One native dialog accepts at most 64 inputs per batch, registers opaque input handles, and inspects up to four files concurrently through the bounded FFprobe route. Each returned row states whether inspection succeeded and which target types match its reported audio or video streams; changing the target updates every row's ready or incompatible state without trusting the filename extension.

Queueing opens one native output-directory picker for all eligible rows. The main process derives sanitized, collision-free names beneath that chosen directory and returns opaque output handles rather than a directory path. Converter argv uses no-overwrite mode, so an output created after planning causes a visible job failure instead of being replaced. Every eligible row is attempted, while unsupported rows remain visible and the result reports queued, failed-to-queue, and skipped counts. Successfully queued rows leave the converter list; failed and unsupported rows remain available for correction or retry.
...
Missing FFprobe, malformed JSON, oversized output, timeouts, unsupported catalog/help kinds, invalid names, empty runtime results, expired file handles, selection overflow, incompatible stream types, output-name exhaustion, and partial enqueue failures are reported rather than replaced with mock data. Cancelling the output-folder picker queues nothing. Probe exports fail if the destination handle is not an output handle, the format is unsupported, the bounded inspection snapshot is no longer available, serialization exceeds 32 MiB, or XML cannot faithfully represent a value.

## Security considerations

Conversion uses opaque handles and structured arrays rather than renderer paths or shell strings. The renderer cannot select an arbitrary executable, inspect a filesystem path, choose an output path directly, or enumerate the selected directory.
...
The trusted batch path is source-wired at implementation commit `033a28e`. Under the ultra-speed delivery boundary, no test, lint, static-analysis, accessibility, runtime interaction, packaging, or capture workflow was run for that change. Batch selection, inspection, target switching, output planning, partial enqueue behavior, and generated files remain unverified in a running build.
