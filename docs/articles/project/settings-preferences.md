...
Renderer state is stored under a versioned local-storage key. Job parallelism accepts one through four, matching the scheduler's enforced bound; lowering it does not interrupt work that is already running, while raising it lets the scheduler fill newly available slots. Appearance values are applied through CSS custom properties. The shipped window minimum is 960 by 640, with a default size of 1440 by 900. The application shell uses local CSS and does not load Google Fonts or a CDN.
