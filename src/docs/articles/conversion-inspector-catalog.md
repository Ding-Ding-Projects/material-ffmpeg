...
Source wiring connects the visual-output cards to native input and output selection, typed argument construction, opaque-handle conversion, and the durable job queue. Source inspection also confirms the snapshot-bound Inspector export route and its serialization bounds. This ultra-speed pass ran no tests, lint, type checks, static analysis, accessibility checks, captures, package execution, live GIF or thumbnail job, media probe, catalog enumeration, help lookup, or export. The source routes are present; packaged, runtime, and visual behavior remain unverified.
