...
Source wiring connects the visual-output cards and typed filtergraph lanes to native input and output selection, typed argument construction, opaque-handle conversion, and the durable job queue. Source inspection also confirms the snapshot-bound Inspector export route and its serialization bounds. Every runtime-declared inventory kind has bounded local search and entry help retrieval with visible unavailable/error states. This ultra-speed pass ran no tests, lint, type checks, static analysis, accessibility checks, captures, package execution, live GIF, thumbnail, or filtergraph job, media probe, catalog enumeration, help lookup, or export. The source routes are present; packaged, runtime, and visual behavior remain unverified.
